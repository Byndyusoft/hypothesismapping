...
Формат цели:
1. Описание цели – текстовое описание того, куда мы идем. Например, интегрировать наш продукт в повседневную жизнь станций техобслуживания.
2. Метрики – параметры и их числовые значения, которые покажут нам результативность нашего движения к цели. К каждой метрике нужно добавить текущее значение и желаемое. Например, число пользователей ежедневно увеличивается в 2 раза (сейчас на 10% в день).
3. Балансирующие метрики – параметры и их числовые значения, которые покажут нам, что мы не сделали хуже, чем было до начала работы по стратегии. Например, стоимость привлечения клиента не превышает ₽1000.

Значения метрик должны быть измеримы и оценены максимально объективно. Исключение только, если вы делаете Карту гипотез персонально для человека. В этом случае оценка метрик может быть субъективной. Например, научиться играть на гитаре на уровне 8 из 10, а сейчас умею на 3.

При необходимости метрики можно [вынести на отдельный уровень](https://github.com/Byndyusoft/hypothesismapping/blob/main/topology.md#2-метрики-цели-на-отдельном-уровне).
