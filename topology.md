...
### Особенность
Визуализируем статусы задач. Статус задачи относится только к исполнению и не
должен одновременно обозначать решение о ресурсе или результат проверки
гипотезы.
...
Если задачи ведутся во внешней рабочей системе, на карте достаточно хранить
устойчивую связь и нужное для стратегического решения представление. Регулярно
проверяйте не только число связанных карточек, но и фактический расход ресурса:
см. [«Связь стратегии с операционкой»](strategyoperations.md).
...
В модели данных тег должен оставаться представлением структурной связи, а не
просто совпадающим текстом. Иначе переименование элемента разорвёт трассировку,
а автоматическая проверка не отличит ссылку от случайного упоминания.
